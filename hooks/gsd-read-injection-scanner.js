...
// Trusted local roots — files here are your own content, never external.
// Anything outside these roots (Downloads, /tmp, scraped output, etc.) gets scanned.
const TRUSTED_ROOTS = [
  '__HOME_DIR__/.claude/',
  '__HOME_DIR__/Documents/Claude Projects/',
];

function isTrustedPath(filePath) {
  const p = filePath.replace(/\\/g, '/');
  return TRUSTED_ROOTS.some(root => p.startsWith(root));
}
...
process.stdin.on('end', () => {
  ...
  try {
    ...
    if (isTrustedPath(filePath) || isExcludedPath(filePath)) {
      process.exit(0);
    }
